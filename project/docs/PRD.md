...
>
> **아직 미구현(향후 과제):** PII 마스킹, Row-Level Security. (하이브리드 검색 S2는 평가 실험으로만 유지 — 프로덕션은 벡터 단독. 상세: rag_evaluation_plan.md §5.2)
...
| S2 | **하이브리드 검색 실험** | BM25 키워드 + Vector 유사도 결합(RRF). 현재 프로덕션 기본값은 평가 결과에 따라 벡터 단독 유지. |
...
- 13주차 베이스라인 측정 → 14주차 청크 메타데이터/주변 청크 확장/재랭커 변경 후 개선폭 비교
