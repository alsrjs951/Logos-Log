...
label_qrels.py 로 만든 정답지에 대해 세 가지 검색 전략의 순위 품질을 비교한다:
  - vector   : bge-m3 임베딩 + $vectorSearch (현재 프로덕션)
  - keyword  : $text 키워드 검색
  - hybrid   : 둘을 Reciprocal Rank Fusion(RRF)으로 결합
...
def main():
    ...
    if not os.path.exists(QRELS_PATH):
        print(f"[!] 정답지 없음: {QRELS_PATH}\n    먼저 label_qrels.py 를 실행하세요.", file=sys.stderr)
        sys.exit(2)

    cases = json.load(open(GOLDEN_PATH, encoding="utf-8"))["cases"]
    qrels = json.load(open(QRELS_PATH, encoding="utf-8"))["qrels"]

    from services.rag_service import RAGService
    from db import get_db
    from pooling import vector_candidates, keyword_candidates, rrf_fuse
    rag, db = RAGService(), get_db()

    strategies = ("vector", "keyword", "hybrid", "hybrid_vw")
    ...
    for c in cases:
        cid = c["id"]
        grades = {k: int(v) for k, v in qrels.get(cid, {}).items()}
        ...
        eq = asyncio.run(rag._expand_query(c["question"]))
        vec = [x["id"] for x in vector_candidates(rag, eq, limit=20)]
        kw = [x["id"] for x in keyword_candidates(db, eq, limit=20)]
        hyb = rrf_fuse([vec, kw])
        hyb_vw = rrf_fuse([vec, kw], weights=[2.0, 1.0])  # 벡터 가중 하이브리드

        ranked = {"vector": vec, "keyword": kw, "hybrid": hyb, "hybrid_vw": hyb_vw}
        for s in strategies:
            r = ranked[s]
        ...
        print(f"  · [{cid}] 정답 {len(rel)}개 | "
              f"R@{TOPK} vec={recall_at_k(vec,rel,TOPK):.2f} kw={recall_at_k(kw,rel,TOPK):.2f} "
    ...
    print("  하이브리드 − 벡터(프로덕션) 개선폭:")
    for m in metrics:
        d = _avg(acc["hybrid"][m]) - _avg(acc["vector"][m])
        print(f"    {m:>10}: {d:+.3f}")
    print("=" * 64)
