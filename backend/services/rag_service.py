        ...
        if is_journal:
            if results:
                system_prompt = (
                    ...
                    "[근거 충실성 원칙 — 가장 중요]\n"
                    "• 특정 논문에 귀속시키는 연구 결과·사실·수치는 오직 아래 [학술 논문 내용]에 실제로 담긴 것만 사용하십시오. 논문에 없는 연구 결과·실험·통계를 지어내거나 과장하지 마십시오.\n"
                    "• 인라인 인용 [N]은 해당 논문이 그 문장을 실제로 뒷받침할 때에만 붙이십시오. 일반적 위로·성찰은 인용 없이 자연스럽게 전개하되, 마치 특정 논문의 근거인 것처럼 포장하지 마십시오.\n"
                    "• 제공된 논문이 일기의 고민을 부분적으로만 다룬다면 그 범위 안에서만 학술 근거를 짚고, 빈 곳은 일반 의미치료·긍정심리학 원칙으로 보완하되 단정적 연구 주장으로 비약하지 마십시오.\n"
                    "• 확신이 없으면 단정하지 말고 '~일 수 있습니다', '~연구가 시사합니다' 같은 신중한 표현을 사용하십시오.\n\n"
                    f"{value_profile}"
                    f"[학술 논문 내용]\n{context_text}"
                )
            else:
            ...
            if results:
                system_prompt = (
                    ...
                    "• 특정 논문에 귀속시키는 연구 결과·사실·수치는 오직 아래 [학술 논문 내용]에 실제로 담긴 것만 사용하십시오. 논문에 없는 연구 결과·실험·통계를 지어내거나 과장하지 마십시오.\n"
                    "• 인라인 인용 [N]은 해당 논문이 그 문장을 실제로 뒷받침할 때에만 붙이십시오. 일반적 위로·성찰은 인용 없이 자연스럽게 전개하되, 마치 특정 논문의 근거인 것처럼 포장하지 마십시오.\n"
                    "• 제공된 논문이 사용자의 고민을 부분적으로만 다룬다면 그 범위 안에서만 학술 근거를 짚고, 빈 곳은 일반 의미치료·긍정심리학 원칙으로 보완하되 단정적 연구 주장으로 비약하지 마십시오.\n"
                    "• 확신이 없으면 단정하지 말고 '~일 수 있습니다', '~연구가 시사합니다' 같은 신중한 표현을 사용하십시오.\n\n"
                    f"{value_profile}"
                    f"[학술 논문 내용]\n{context_text}"
                )
